...
function KeystoneOnboardingModal({ open, redirectUrl, ctaLabel, onClose }) {
    const [phase, setPhase] = useState("form"); // form | submitting | transition
    ...
    const onSubmit = async (e) => {
        ...
        // Cinematic full-screen brand transition
        setPhase("transition");
        // Total transition: ~2.2s — gives time to read the headline & build anticipation
        setTimeout(() => {
            window.open(finalRedirect, "_blank", "noopener,noreferrer");
            // close after a brief breath so the new tab takes focus and the user lands smoothly
    ...
    if (phase === "transition") {
        return (
            <div
                data-testid="keystone-cinematic-transition"
                className="fixed inset-0 z-[120] flex items-center justify-center px-6"
                role="dialog"
                aria-modal="true"
            >
                {/* Dark backdrop */}
                <div className="absolute inset-0 bg-ink animate-fade-in" />
                {/* Gold orbs */}
                <div className="pointer-events-none absolute inset-0 overflow-hidden">
                    <div className="absolute top-1/3 left-1/2 -translate-x-1/2 h-[640px] w-[640px] rounded-full bg-gold/15 blur-[160px] animate-fade-in" />
                    <div
                        className="absolute -bottom-32 -right-32 h-[400px] w-[400px] rounded-full bg-gold/10 blur-[140px] animate-fade-in"
                ...
                {/* Center content */}
                <div className="relative text-center max-w-2xl mx-auto animate-fade-up">
                    <img
                        src="/crluys-logo.png"
                        alt="CrLuys Lifestyle"
                        className="brand-logo-soft h-20 md:h-24 w-auto mx-auto mb-10 animate-fade-up"
                        draggable={false}
                    />
                    <div className="inline-flex items-center gap-3 mb-7 px-4 py-2 rounded-full glass">
                        <span className="h-1.5 w-1.5 rounded-full bg-gold animate-pulse" />
                        <span className="text-[10px] uppercase tracking-[0.45em] text-gold/95">
                            CrLuys × Keystone Investors Club
                        </span>
                    </div>
                    <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl text-cream leading-[1.0] tracking-tight">
                        Financial Freedom
                        <br />
                    ...
                    <div
                        className="mt-10 inline-flex items-center gap-3 text-sm uppercase tracking-[0.4em] text-gold/90 animate-fade-up"
                    ...
                    <p
                        className="mt-6 text-xs text-cream/45 animate-fade-up"
                        style={{ animationDelay: "0.8s" }}
                    >
                        A new tab is opening · You can return here anytime
                    </p>
                </div>
            </div>
        );
            ...
            >
                ...
                {phase === "form" && (
                    <button
                        ...
                        aria-label="Close"
                        className="absolute top-5 right-5 h-9 w-9 inline-flex items-center justify-center rounded-full border border-gold/25 text-cream/70 hover:text-gold hover:border-gold transition-colors z-10"
                    >
                        <X className="h-4 w-4" />
                    </button>
                )}
